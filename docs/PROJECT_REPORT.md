...
4. **可扩展**：模型抽象层预留 Live2D/素材模型接入点，支持换装换表情
...
- **Live2D / 自制素材模型 = 再写一个 trait 实现**，主程序状态机、菜单、交互零改动
...
| PetModel 接口 | 预留 Live2D/素材模型接入点（见 §3.2） |
...
| Live2D 未实现 | 仅接口预留 | 按 PetModel trait 实现 |
...
4. Live2D 后端（cubism web/core 接入 PetModel）
...
- Live2D 预留说明：PetModel trait 的 Pose 已语义化状态（走/坐/爬/伸懒腰/舔毛/吃/睡…），Live2D 实现将状态映射到 motion 即可
