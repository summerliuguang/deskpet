...
单文件 exe 约 10.8MB（内含 7.0MB Fusion Pixel 像素中文字库 TTF 版），运行内存 ~10MB，
...
**AI 与语音**
- 聊天窗：内置 Fusion Pixel 像素中文字库（TTF 版，SIL OFL 可商用）、系统输入法、回车发送、历史记录
...
- **多屏适配**：散步/爬墙/弹跳边界跟随猫所在的显示器，跨屏拖动自由；
  跨屏落点带 1/4 边长容差防突跳；显示器热插拔/改分辨率 2 秒内自动
  重枚举，宠物所在屏被拔掉时自动搬回有效屏幕
