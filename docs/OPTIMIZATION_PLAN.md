...
> **状态（2026-09-18）：四批全部实施并验证通过**（提交 842bc11 / bb77f43 / 7e25acc / 706c8d6 + 验证修复 5a504d6）。
> 实施偏差：A3 未做 HashMap 统一路由（menu 的 stay/outcome 特例多，改为收敛三个横切操作）；
> A4 未改 draw_clipped 签名（收益低于成本）。附带的意外修复：menu 圆角判定从未生效，rounded_inside
> 改标准实现后菜单首次显示真圆角。真机待验：拖动跟手/双击后拖动/跨 DPI/爬墙姿态/大模型切换/断网文案/拔副屏。
